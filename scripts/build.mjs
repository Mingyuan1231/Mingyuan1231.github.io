...
const person = {
  ...
  sameAs:[config.github,config.scholar,config.orcid],
  memberOf:[{'@type':'Organization',name:'Krastanov Lab',url:'https://lab.krastanov.org/'},{'@type':'Organization',name:'Center for Quantum Networks (NSF ERC)',url:config.cqn}], knowsAbout:['Entanglement distillation','Quantum error correction','Graph-state purification','Quantum networks','Stabilizer formalism','Clifford circuits','Combinatorial optimization'],
};
const routes = [];
...
<footer class="site-footer"><div class="wrap footer-inner"><p>Mingyuan Wang · University of Massachusetts Amherst</p><p><a href="mailto:${config.email}">Email</a> &nbsp; / &nbsp; <a href="${config.github}">GitHub</a> &nbsp; / &nbsp; <a href="${esc(config.scholar)}">Scholar</a> &nbsp; / &nbsp; <a href="${config.orcid}">ORCID</a> &nbsp; / &nbsp; <a href="/credits/">Sources & credits</a></p></div></footer></body></html>\n`;
}
...
for (const p of papers) {
  ...
  const abstract = p.abstract ? `<h2 id="abstract">Abstract</h2><p class="abstract">${esc(p.abstract)}</p><p class="source">Author abstract from the <a href="${p.abstractSource || `https://arxiv.org/abs/${p.arxiv}`}">${esc(p.abstractSourceLabel || 'arXiv preprint')}</a>, by ${esc(p.authors.join(', '))}. ${p.abstractLicense ? `<a href="${p.abstractLicense}">CC BY 4.0</a>; mathematical notation reformatted for the web.` : 'Reproduced at the author’s request.'}</p>` : `<h2 id="summary">Research summary</h2><p>${esc(p.overview)}</p><p class="source">${p.journal ? `Read the <a href="https://doi.org/${p.doi}">journal article</a> or the <a href="https://arxiv.org/abs/${p.arxiv}">author preprint on arXiv</a>.` : `Summary of the preprint. Read the <a href="https://arxiv.org/abs/${p.arxiv}">full author abstract on arXiv</a>.`}</p>`;
  ...
  await page(route,`${p.title} | Mingyuan Wang`,p.summary,
    ...
    {'@type':'ScholarlyArticle','@id':absolute(route),url:absolute(route),headline:p.title,name:p.title,author:p.authors.map(a=>a===config.name ? person : {'@type':'Person',name:a}),datePublished:p.date,description:p.summary,...(p.abstract ? {abstract:p.abstract,...(p.abstractLicense ? {license:p.abstractLicense} : {})} : {}),creativeWorkStatus:p.status,identifier:[{'@type':'PropertyValue',propertyID:'DOI',value:p.doi},{'@type':'PropertyValue',propertyID:'arXiv',value:p.arxiv}],sameAs:p.journal ? `https://doi.org/${p.doi}` : `https://arxiv.org/abs/${p.arxiv}`,...publicationSchema});
...
await page('/research/sequence-control/','Partial Distillation & PairMoment Sequence Control | Mingyuan Wang','Sequence control for partial graph-state distillation under correlated noise. PairMoment uses an O(n²) state representation of single-vertex errors and pair correlations.',
  `${heading('Current research','Sequence control under correlated noise','Partial graph-state distillation and the PairMoment reduced model.')}${(await content('sequence-control')).replace('{{SEQUENCE_ABSTRACT}}',esc(academic.manuscripts[0].abstract))}`);
